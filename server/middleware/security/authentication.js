  ...
  User.findByPk(userid)
    .then(user => {
      ...
      if (req.jwtPayload.lastLoginAt) {
         ...
         * and lastLoginAt is null in the db; so we allow them in if both
         * conditions are met.
         */
        if (req.jwtPayload.lastLoginAt === 'firstLogin' && !user.lastLoginAt) {
          ...
          /**
           * For normal logins, we check that the lastLoginAt in the JWT matches
           * the lastLoginAt in the db. If so, we allow the user to log in, and
